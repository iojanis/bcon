...
impl ClientServer {
    ...
    async fn handle_client_connection(
        ...
        let connection_id = uuid::Uuid::new_v4().to_string();
        let mut authenticated_token: Option<ValidatedClientToken> = None;
        let mut current_role = crate::auth::ClientRole::Guest;

        // Send initial server info message (like Deno implementation)
                    ...
                    match serde_json::from_str::<IncomingMessage>(&text) {
                        Ok(message) => {
                            ...
                            if message.is_auth_message() && authenticated_token.is_none() {
                                ...
                                // Update role if authentication successful
                                if let Some(ref token) = authenticated_token {
                                    current_role = token.role.clone();
                                }
                            } else {
                                // Handle regular messages (including from guests)
                                // Check rate limit
                                let rate_result = rate_limiter.check_client_rate_limit(
                                    &client_ip,
                                    &current_role,
        ...
        info!("Client {} disconnected from {}", connection_id, client_addr);
        Ok(())
    }
    ...
    async fn handle_auth_message(
        ...
        match auth_service.verify_client_token(token) {
            Ok(validated_token) => {
                let user_info = crate::message::UserInfo {
                    username: validated_token.username.clone().unwrap_or_else(|| "Unknown".to_string()),
                    role: validated_token.role.as_str().to_string(),
                ...
                let success_response = serde_json::json!({
                    ...
                    "user": user_info,
                    "message": format!("Logged in as {}", user_info.username),
                    "timestamp": std::time::SystemTime::now()
                        .duration_since(std::time::UNIX_EPOCH)
                        .unwrap()
                        .as_secs(),
