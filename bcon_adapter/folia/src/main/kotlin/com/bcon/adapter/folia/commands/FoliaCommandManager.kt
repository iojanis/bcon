package com.bcon.adapter.folia.commands
...
/**
 * Folia-specific command manager that integrates with Bukkit's command system
 * Handles registration and execution of dynamic commands in Folia with region-aware scheduling
 */
class FoliaCommandManager(
    private val plugin: JavaPlugin,
    private val coreManager: DynamicCommandManager
) {
    
    private val logger = Logger.getLogger(FoliaCommandManager::class.java.simpleName)
    ...
    private fun registerExistingCommands() {
        logger.info("Registering existing dynamic commands with Folia")
        
        coreManager.getAllCommands().forEach { (name, definition) ->
            registerBukkitCommand(definition)
        }
        
        logger.info("Registered ${coreManager.getAllCommands().size} dynamic commands with Folia")
    }
     ...
     */
    fun shutdown() {
        logger.info("Shutting down Folia command manager")
        ...
        logger.info("Folia command manager shutdown complete")
    }
...
/**
 * Bukkit command wrapper for dynamic commands (Folia-aware)
 */
    ...
    private fun convertSender(bukkitSender: org.bukkit.command.CommandSender): CommandSender {
        return when (bukkitSender) {
            ...
            is BlockCommandSender -> CommandSender(
                ...
                name = bukkitSender.name,
                type = SenderType.CONSOLE, // Default to CONSOLE
                uuid = "unknown"
            )
        }
    }
